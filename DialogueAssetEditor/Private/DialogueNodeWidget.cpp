...
{	
	...
	DialogueViewportWidget = InArgs._DialogueViewportWidget;
	...
	*/
	+SOverlay::Slot()
		...
		[
			SNew(SSpacer)
			.Size(TAttribute<FVector2D>::Create(TAttribute<FVector2D>::FGetter::CreateSP(DialogueViewportWidget, &SDialogueViewportWidget::GetNodeMinSize)))
		]
	...
	*/
	...
	+SOverlay::Slot()
		...
		.WrapTextAt(TAttribute<float>::Create(TAttribute<float>::FGetter::CreateSP(DialogueViewportWidget, &SDialogueViewportWidget::GetNodeTextWrapLength))) // 250 by default at 1:1 zoom level
		.Font(TAttribute<FSlateFontInfo>::Create(TAttribute<FSlateFontInfo>::FGetter::CreateSP(DialogueViewportWidget, &SDialogueViewportWidget::GetNodeFont)))
		]
	*/
	/*
...
FMargin SDialogueNodeWidget::GetTextMargin() const
{
	...
	float marg10 = 10 * DialogueViewportWidget->GetZoomAmount();
	float marg17 = 16 * DialogueViewportWidget->GetZoomAmount();

	if (DialogueViewportWidget->GetZoomLevel() <= 4)
	{
		return FMargin(0, 0, 0, 0);
	}
...
void SDialogueNodeWidget::OnMouseEnter(const FGeometry & MyGeometry, const FPointerEvent & MouseEvent)
{
	...
	if (Dialogue->isLinking && !DialogueViewportWidget->isLinkingAndCapturing)
	{
		// we shouldn't link a node to itself
		if (Id != Dialogue->Data[Dialogue->LinkingFromIndex].NodeId)
		{
...
{
	return FDialogueEditorStyle::Get()->GetBrush("NodeStyle");
}
...
FReply SDialogueNodeWidget::OnDragDetected(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	...
	{
		DialogueViewportWidget->isPanning = true;
	}
	if (MouseEvent.IsMouseButtonDown(EKeys::LeftMouseButton))
	{
		if (DialogueViewportWidget->bBreakingLinksMode)
			DialogueViewportWidget->bBreakingLinksMode = false;
		...
		DialogueViewportWidget->draggingOffset = FVector2D(MyGeometry.GetLocalSize().X / 2, MyGeometry.GetLocalSize().Y / 2) - MyGeometry.AbsoluteToLocal(MouseEvent.GetScreenSpacePosition());
		DialogueViewportWidget->StartDraggingIndex(NodeIndex);
	}
...
FReply SDialogueNodeWidget::OnMouseButtonUp(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	...
	if (MouseEvent.GetEffectingButton() == EKeys::LeftMouseButton)
	{
		if ((bJustDoubleClicked || (DialogueViewportWidget->SelectedNodes.Num() == 1 && DialogueViewportWidget->SelectedNodes.Contains(Id) /*&& NodeTextBlock->IsHovered()*/)) && Id != 0)
		{
			bJustDoubleClicked = false;

			// Don't enter Edit mode if we're at -6 zoom or lower.
			/*
			if (DialogueViewportWidget->GetZoomLevel() > 6)
			{
				NodeField->SetText(GetFieldText());
				DialogueViewportWidget->EditTextNode = Id;
	...
	else if (MouseEvent.GetEffectingButton() == EKeys::RightMouseButton)
	{
		...
		if (!DialogueViewportWidget->SelectedNodes.Contains(Id))
		{
			DialogueViewportWidget->SelectNodes(Id);
		...
		FUIAction DeleteAction(FExecuteAction::CreateSP(DialogueViewportWidget, &SDialogueViewportWidget::DeleteOneNode, Id));
		FUIAction DeleteAllAction(FExecuteAction::CreateSP(DialogueViewportWidget, &SDialogueViewportWidget::DeleteSelected));
		...
		*/
		...
		if (DialogueViewportWidget->SelectedNodes.Num() == 1 && DialogueViewportWidget->SelectedNodes[0] == Id)
		{
		...
		*/
		...
		if (DialogueViewportWidget->SelectedNodes.IsValidIndex(0) && DialogueViewportWidget->SelectedNodes.IsValidIndex(1)) // if multiple are selected
		{
...
{
	DialogueViewportWidget->bBreakingLinksMode = true;
	DialogueViewportWidget->breakingLinksFromId = Id;
}
...
void SDialogueNodeWidget::BreakLinksWithNode()
{
	...
	if (DialogueViewportWidget->breakingLinksFromId == Id || (!Dialogue->Data[NodeIndex].Links.Contains(DialogueViewportWidget->breakingLinksFromId) && !DialogueViewportWidget->GetNodeById(DialogueViewportWidget->breakingLinksFromId).Links.Contains(Id)))
	{
		return;
	}
	...
	{
		return (param1 == DialogueViewportWidget->breakingLinksFromId);
	});

	int32 BreakingFromNodeIndex = DialogueViewportWidget->NodeIdsIndexes.FindRef(DialogueViewportWidget->breakingLinksFromId);
...
void SDialogueNodeWidget::OnDeleteNode(bool withRefresh = true)
{
	...
	Dialogue->Data.RemoveAt(DialogueViewportWidget->NodeIdsIndexes.FindRef(Id));
	DialogueViewportWidget->DeselectNode(Id);
...
void SDialogueNodeWidget::OnAddPcAnswer()
{
	...
	if (Dialogue->Data[index].Links.Num() == 0)
	{
		...
		NewNode.Coordinates.Y = Dialogue->Data[index].Coordinates.Y + (NodeSize.Y + DialogueViewportWidget->NodeWidgets[0]->NodeSize.Y) / 2.f / DialogueViewportWidget->GetZoomAmount() + 75.f; // half the size of current widget + half the size of first widget + desired distance
	}
	else
	{
		int32 lastIndex;
		...
		{
			int tempIndex = DialogueViewportWidget->NodeIdsIndexes.FindRef(param1);
		...
		if (linkToLastId != INDEX_NONE)
		{
			...
			lastIndex = DialogueViewportWidget->NodeIdsIndexes.FindRef(lastId);

			NewNode.Coordinates.X = Dialogue->Data[lastIndex].Coordinates.X + (DialogueViewportWidget->NodeWidgets[lastIndex]->NodeSize.X + DialogueViewportWidget->NodeWidgets[0]->NodeSize.X) / 2.f / DialogueViewportWidget->GetZoomAmount() + 50.f; // half the size of the preceding widget + half the size of the new widget + desired distance
	...
	DialogueViewportWidget->SpawnNodes(NewNode.id);
}
...
void SDialogueNodeWidget::OnAddLink()
{
	...
	DialogueViewportWidget->isLinkingAndCapturing = true;
	Dialogue->LinkingFromIndex = NodeIndex;
	Dialogue->LinkingCoords = Dialogue->Data[NodeIndex].Coordinates;
	DialogueViewportWidget->ForceSlateToStayAwake();
}
	...
	for (auto Node : Dialogue->Data)
	{
		...
		for (auto Link : Node.Links)
		{
			if (Dialogue->Data[i].Links[j] == Id) // if a link to current node has been found, then perform a sort on the links
			{
				...
				{
					int32 index1 = DialogueViewportWidget->NodeIdsIndexes.FindRef(id1);
					int32 index2 = DialogueViewportWidget->NodeIdsIndexes.FindRef(id2);
