...
void SDialogueViewportWidget::Construct(const FArguments& InArgs, TSharedPtr<FDialogueAssetEditor> InDialogueEditor)
{		
	if (!ZoomLevels.IsValid())
	{
		ZoomLevels = MakeShareable(new FDialogueFixedZoomLevelsContainer());
	}
...
FCursorReply SDialogueViewportWidget::OnCursorQuery(const FGeometry& MyGeometry, const FPointerEvent& CursorEvent) const
{
	...
	if (isSelectingMultiple)
	{
		...
		for (auto Node : Dialogue->Data)
		{
			...
			{
				temp_selectedNodeIds.Add(Node.NodeId);
			}
			nodeIndex++;
		}
...
void SDialogueViewportWidget::StartDraggingIndex(int32 NodeIndex)
{
	...
	{
		SelectNodes(Dialogue->Data[NodeIndex].NodeId);
	}
	else
	{
		if (!SelectedNodes.Contains(Dialogue->Data[NodeIndex].NodeId))
		{
			DeselectAllNodes();
			SelectNodes(Dialogue->Data[NodeIndex].NodeId);
			ForceRefresh();
...
void SDialogueViewportWidget::OnAddNodeClicked()
{
	...
	NewNode.NodeId = Dialogue->NextNodeId;
	...
	Dialogue->NextNodeId++;
	Dialogue->Data.Add(NewNode);
	Dialogue->CurrentNodeId = NewNode.NodeId; //select the new node
	SpawnNodes(NewNode.NodeId);
}
		...
		for (auto Link : Node.Links) // for each link
		{
			...
			if (linkIndex != -1)
			{
				...
				if (Node.Coordinates.Y > Dialogue->Data[linkIndex].Coordinates.Y)
				{
					...
					if (SelectedNodes.Num() == 1 && SelectedNodes[0] == Node.NodeId)
					{
...
FVector2D SDialogueViewportWidget::ComputeSplineTangent(const FVector2D& Start, const FVector2D& End) const
{
	...
	const float ClampedTensionX = FMath::Min<float>(FMath::Abs<float>(DeltaPos.X), bGoingForward ? GraphEditorSettings->ForwardSplineHorizontalDeltaRange : GraphEditorSettings->BackwardSplineHorizontalDeltaRange);
	const float ClampedTensionY = FMath::Min<float>(FMath::Abs<float>(DeltaPos.Y), bGoingForward ? GraphEditorSettings->ForwardSplineVerticalDeltaRange : GraphEditorSettings->BackwardSplineVerticalDeltaRange);

	if (bGoingForward)
	{
		return (ClampedTensionX * GraphEditorSettings->ForwardSplineTangentFromVerticalDelta) + (ClampedTensionY * GraphEditorSettings->ForwardSplineTangentFromVerticalDelta);
	}
	else
	{
		return (ClampedTensionX * GraphEditorSettings->BackwardSplineTangentFromHorizontalDelta) + (ClampedTensionY * GraphEditorSettings->BackwardSplineTangentFromVerticalDelta);
	}
...
void SDialogueViewportWidget::SelectUnderCursor() const 
{
	...
	for (auto Node : Dialogue->Data)
	{
		...
		{
			if ( (CurrentSelection == ESelectionType::Type::WithShift || CurrentSelection == ESelectionType::Type::WithCtrl) && !SelectedNodes.Contains(Node.NodeId) )
			{
				SelectedNodes.Add(Node.NodeId);
				NodeWidgets[nodeIndex]->isSelected = true;
			}
			else if (CurrentSelection == ESelectionType::Type::WithCtrl && SelectedNodes.Contains(Node.NodeId))
			{
